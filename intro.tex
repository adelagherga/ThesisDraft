...
f(x,y) = cp_1^{z_1}\cdots p_v^{z_v}, \quad (x,y) = 1,
\end{equation}
...
If there is a solution in integers $x,y, n$ and $m$ to the equation 
\begin{equation}\label{Introduction:GoormaghtighAndCondition}
\frac{x^m-1}{x-1} = \frac{y^n-1}{y-1}, \quad \text{ where } \quad \gcd(m-1,n-1) = d > 1,
\end{equation}
then
...
$\max \{ x, y, m \} < c$.
\end{theorem}
By refining our approach, new results from computational Diophantine approximation enable us to achieve the complete solution of equation \eqref{Introduction:GoormaghtighAndCondition} for small fixed values of $n$. \\

\begin{theorem} \label{IntroductionTheorem:Goormaghtigh2Eq}
If there is a solution in integers $x,y$ and $m$ to equation \eqref{Introduction:GoormaghtighAndCondition} with $n \in \{ 3, 4, 5 \}$, then
\[(x,y,m,n) = (2,5,5,3)  \; \mbox{ and } \; (2,90,13,3).\]
\end{theorem}

In the case $n = 5$ of Theorem \ref{IntroductionTheorem:Goormaghtigh2Eq}, ``off-the-shelf'' techniques for finding integral points on models of elliptic curves or for solving {\it Ramanujan-Nagell} equations of the shape $F(x)=z^n$ (where $F$ is a polynomial and $z$ a fixed integer) do not apparently permit the full resolution of this problem in a reasonable amount of time. Instead, we sharpen the existing techniques of \cite{TW} and \cite{Ham} for solving Thue-Mahler equations and specialize them to this problem. The work presented on Goormaghtigh equations is joint work with Dr. M. Bennett and Dr. D. Kreso and has been submitted for publication \cite{BeGhKr}. 
