...
Following Remark \ref{rem:uv}, for the remainder of this chapter, we assume that $u_i \neq 0$ for all $i = 1, \dots, v$. As in \ref{subsec:SUnitEquation}, we fix a complete set of fundamental units $\{\eps_1, \dots, \eps_r\}$ of $\mathcal{O}_K$. Recall $r = s + t -1$, where $s$ denotes the number of real embeddings of $K$ into $\mathbb{C}$ and $t$ denotes the number of complex conjugate pairs of non-real embeddings of $K$ into $\mathbb{C}$. A quick computation in Maple shows that  
...
and
\[ b_{v+2} = a_1\]
of \autoref{subsec:FirstStepsSmallBoundsGE} frequently. It will therefore be convenient to let $B_l$ denote the current best upper bound for $|b_l|$ for $l = 1, \dots, v+2$. Then
